...
var sortArray = function (nums) {
  if (nums.length === 0 || nums.length === 1) {
    ...
    let firstHalf = sortArray(nums.slice(0, half))
    let secondHalf = sortArray(nums.slice(half))
...
var mergeSortedArrays = (arr1, arr2) => {
  ...
  while (pointer1 < arr1.length || pointer2 < arr2.length) {
    if (pointer2 === arr2.length || arr1[pointer1] <= arr2[pointer2]) {
      ...
      pointer1++;
    } else if (pointer1 === arr1.length || arr2[pointer2] <= arr1[pointer1]) {
...
// console.log(mergeSortedArrays([-1, 5, 7, 8], [0, 3, 5]))
console.log(sortArray([3, 10, -1, 4, 3, 2, 11, 13]))